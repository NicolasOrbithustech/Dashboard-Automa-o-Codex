...
const DEFAULT_IMAGE_STYLE = "premium clean 3D editorial illustration, not photorealistic, no people, no real brands";
...
export async function generateSocialPost(input) {
  const prompt = [
    ...
    "O prompt de imagem deve ser, por padrao, uma ilustracao 3D premium vertical 9:16, sem pessoas, sem marcas registradas e sem texto pequeno ilegivel.",
    "So use foto/fotorrealismo se o operador pedir explicitamente; caso contrario prefira visual limpo, controlado e menos sujeito a rostos/maos estranhos.",
...
export function buildImagePrompt(input) {
  ...
  const asksForPhoto = /(foto|fotorreal|photoreal|realista|pessoa|people|person|mulher|homem|rosto|face|hands|maos|mãos)/i.test(sourceText);
  const forbidsPeople = /(sem pessoa|sem pessoas|no people|no person|sem rosto|no face|sem humanos|no humans)/i.test(sourceText);
  const mode = asksForPhoto && !forbidsPeople
    ? "controlled photorealistic lifestyle scene with one natural adult, no close-up hands, no distorted face"
  ...
  return [
    ...
    "Primary composition: centered smartphone with a simple abstract survey checklist, generic K coins, generic reward cards, clean premium layout, trustworthy fintech-like polish.",
    "Visual quality rules: balanced spacing, clear focal point, consistent lighting, modern teal/gold/navy palette, no clutter, no awkward cropping, no messy UI.",
    "Hard avoid list: real brand logos, copyrighted characters, tiny unreadable interface text, fake app names, distorted hands, uncanny faces, extra fingers, random people in the background, cheap stock-photo look, low quality artifacts.",
    "Text rule: if any text appears, keep it large, short, and legible; otherwise prefer abstract UI bars and icons instead of words.",
