...
const MAX_BYTES = Number(process.env.KOINOPS_UPLOAD_MAX_BYTES || 8 * 1024 * 1024);
...
export function validateUpload(file) {
  ...
  if (!ALLOWED_TYPES.has(file.contentType)) throw new Error("Envie apenas JPG ou PNG.");
  if (file.buffer.length > MAX_BYTES) throw new Error("Imagem grande demais. Use ate 8 MB.");
}
